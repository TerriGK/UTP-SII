...
let limit = 100, alumnosData = [], selectedAsignatura = null, selectedEval = 'A';

// Función para obtener todos los alumnos por bloques
const getAlumnos = async () => {
  loaderTable.style.display = "block";
  alumnosData = []; // Limpiar antes de cargar
  ...
  try {
    ...
    llenarAsignaturas(alumnosData);
    llenarEvaluaciones(alumnosData);
    mostrarAlumnos();
...
// Función para actualizar nombre de profesor
...
const mostrarAlumnos = () => {
  ...
  let content = "", seen = new Set(), counter = 1;
...
// Actualizar datos del alumno vía API
...
const actualizarTodasCalificaciones = async () => {
  ...
  for (const alumno of updates) {
    try {
      ...
      console.error("Error al actualizar alumno:", alumno.numeroalumno, error);
    }
  }
...
// Procesar Excel y marcar diferencias

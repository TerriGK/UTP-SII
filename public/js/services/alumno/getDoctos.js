  ...
  try {
    ...
    if (data.doctos && data.doctos.length > 0) {
      data.doctos.forEach((item) => {
        const entregado = item.ENTREGADO === true || item.ENTREGADO === 'true' || item.ENTREGADO === 1 || item.ENTREGADO === '1';
        ...
        const icono = entregado
          ? '<i class="bi bi-x-circle-fill text-danger fs-3" title="No entregados"></i>'
          : '<i class="bi bi-check-circle-fill text-success fs-3" title="Entregados"></i>';

        const textoEntrega = entregado
          ? `<p class="text-danger"><strong>No entregado</strong></p>`
            ...
            <div class="col-md-3 col-lg-4 col-12 mb-4">
              <div class="card border-0 shadow-sm">
                <div class="card-body text-center">
                  ...
                  ${icono}
                  ${textoEntrega}
                  <button class="btn btn-info mt-2" onclick="viewDocumentDetails('${item.ID_DOCTO}')">Mostrar Información</button>
                </div>
              </div>
            </div>`;
      });

      // Agrega el contador de documentos entregados y no entregados
      content = `<div class="text-center mb-4">
        <h4>Documentos Entregados: ${noEntregados}</h4>
        <h4>Documentos No Entregados: ${entregados}</h4>
      </div>` + content;

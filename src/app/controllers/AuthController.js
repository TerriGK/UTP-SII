...
AuthController.authAlumno = async (req = request, res = response) => {
  const { user } = req.body; // Eliminamos el parámetro password ya que no se usará
  try {
    const alumno = await Alumno.findById(user);
    
    if (!alumno) {
      return handleError(req, res, 'Alumno no encontrado', `Error: Alumno ${user} no existe`);
    }
    
    if (alumno.STATUS.trim() === "S") {
      return handleError(req, res, 'No pueden ingresar los aspirantes');
    }
    
    // Eliminamos la verificación de contraseña
    setSession(req, alumno, 'isAlumno');
    res.redirect('/');
...
AuthController.authAspirante = async (req = request, res = response) => {
  ...
  try {
    if (!user || isNaN(user.trim())) {
      return handleError(req, res, 'El folio debe ser un número válido');
    }
    
    const aspirante = await Aspirante.findById(user.trim());
    
    if (!aspirante) {
      return handleError(req, res, 'Folio de aspirante no encontrado');
    }
    
    if (aspirante.STATUS.trim() !== "S") {
      return handleError(req, res, 'Ya no puede ingresar con el folio proporcionado');
    }
...
AuthController.authProfe = async (req = request, res = response) => {
  ...
  try {
    ...
    if (!profesor) {
      return handleError(req, res, 'Profesor no encontrado', `Error: Profesor ${user} no existe`);
    }
    
    if (!profesor.PASSWORD?.trim()) {
      return handleError(req, res, 'El profesor no tiene una contraseña registrada');
    }
    
    if (password !== profesor.PASSWORD) {
      return handleError(req, res, 'Contraseña incorrecta');
    }
...
AuthController.authAdmin = async (req = request, res = response) => {
  ...
  try {
    ...
    if (!userData) {
      return handleError(req, res, 'Usuario no encontrado', `Error: Usuario ${user} no existe`);
    }
    
    if (userData.ADMINISTRADOR.trim() === "N") {
      return handleError(req, res, 'El usuario no es administrador');
    }

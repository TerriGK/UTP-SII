...
//Aspirante datos 
AlumnosAdminCtr.showByIdAspirante = async (req = request, res = response) => {
        ...
        if (foto) {
          foto(function (err, _name, e) {
            ...
            e.on("end", () => {
              ...
              res.render("alumno/Aspirante/Aspirante-screen", newAlumno);
           
              db.detach();
            });
          });
        } else {
          res.render("alumno/Aspirante/Aspirante-screen", alumno);
        }
      }
    );
  });
...
AlumnosAdminCtr.updateAspirante = async (req = request, res = response) => {
  const body = req.body;
...
AlumnosAdminCtr.updatePhotoAspirante = async (req = request, res = response) => {
  let id = req.params.id;
...
AlumnosAdminCtr.doctosAspirante = async (req = request, res = response) => {
  const alumno = await Alumno.findById(req.params.id);
  res.render("alumno/documentos/doctos-screen", {
    numeroalumno: alumno?.NUMEROALUMNO,
    nombre: alumno?.NOMBRE,
...
AlumnosAdminCtr.boletasAspirante = async (req, res) => {
  const alumnos = await Alumno.findById(req.params.id);
  res.render("alumno/boletas/boletas-screen", alumnos);
};
